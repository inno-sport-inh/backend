...
def convert_personal_training(t) -> dict:
    ...
    r = {
        ...
        "extendedProps": {
            "id": t["id"],
            "can_edit": start_time <= timezone.localtime() <= start_time + settings.TRAINING_EDITABLE_INTERVAL,
            "group_id": t["group_id"],
...
@api_view(["GET"])
def get_schedule(request, sport_id):
    serializer = CalendarRequestSerializer(data=request.query_params)
    serializer.is_valid(raise_exception=True)
    student = getattr(request.user, "student", None)
    trainings = get_sport_schedule(sport_id, student=student)
...
def get_personal_schedule(request):
    serializer = CalendarRequestSerializer(data=request.query_params)
    serializer.is_valid(raise_exception=True)
    ...
    result_dict = {training["id"]: training for training in student_trainings + trainer_trainings}
